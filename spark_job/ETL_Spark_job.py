...
from pyspark.sql.functions import to_json, struct , concat,  regexp_replace
import json
#from uuid import *
...
def import_to_kafka(output):
    schema_dict = {
            'type': 'struct',
        "fields": [
            {"type": "string", "optional": False, "field": "job_id"},
            {"type": "string", "optional": False, "field": "dates"},
            {"type": "int32", "optional": False, "field": "hours"},
            {"type": "int32", "optional": False, "field": "publisher_id"},
            {"type": "int32", "optional": False, "field": "company_id"},
            {"type": "int32", "optional": False, "field": "campaign_id"},
            {"type": "int32", "optional": False, "field": "group_id"},
            {"type": "int32", "optional": False, "field": "disqualified_application"},
            {"type": "int32", "optional": False, "field": "qualified_application"},
            {"type": "int32", "optional": False, "field": "conversion"},
            {"type": "int32", "optional": False, "field": "clicks"},
            {"type": "int32", "optional": False, "field": "bid_set"},
            {"type": "int32", "optional": False, "field": "spend_hour"},
            {"type": "string", "optional": False, "field": "sources"}
        ],
        "optional": False,
        "name": "metrics"
    }
    schema_json_str = json.dumps(schema_dict, ensure_ascii=False)
    payload_json = to_json(struct(*[col(c).alias(c) for c in output.columns]))
    final_message = concat(
        ...
        payload_json,
        lit('}'))
    final_message_no_backslashes = regexp_replace(final_message, '\\\\', '')
    final_output_with_message = output.withColumn("value", final_message_no_backslashes)
    final_output_with_message.selectExpr("CAST(job_id AS STRING) AS key", "CAST(value AS STRING) AS value")\
        .write\
        .format("kafka")\
...
def get_latest_time_psql(url,driver,user,password):
    sql = """(select max(latest_update_time) from events) data"""
    mysql_time = spark.read.format('jdbc').options(url=url, driver=driver, dbtable=sql, user=user, password=password).load()
    mysql_time = mysql_time.take(1)[0][0]
    if mysql_time is None:
        psql_latest = '1998-01-01 23:59:59'
    else :
        psql_latest = mysql_time.strftime('%Y-%m-%d %H:%M:%S')
    return psql_latest
...
#    psql_time = get_latest_time_psql(url,driver,user,password)
#    print(f'PostgreSQL latest time is {psql_time}')
